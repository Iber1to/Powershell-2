...
$CheckRecord = Get-HostsRecord -HostName "jira.ClientUtility.es"

If($CheckRecord){
    Write-Output "Found record for jira.ClientUtility.es con valor $checkRecord"
    Exit 0
}
Else{
    Write-Output "No record for jira.ClientUtility.es"
    Exit 1
}
Stop-Transcript