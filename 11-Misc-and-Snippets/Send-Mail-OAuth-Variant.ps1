...
param(
    ...
    [ValidateNotNull()]
    [int]$expireInDays,    # From Address, eg "IT Support <user@example.com>"
    [switch]$logging,
    # Log File Path
    [string]$logPath,
    # Testing Enabled
...
$cabecera = "C:\password\Cabecera.png" #ESTE ES EL ARCHIVO QUE TENEMOS QUE TOCAR PARA CAMBIAR EL ICONO
...
#import credential
$username = "automation@contoso.com"
...
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress, otherMailbox | Where-Object { $_.passwordexpired -eq $false } # Start Modify CCopy
# Count Users
...
foreach ($user in $users)
{
    ...
    $ccemailaddress = $user.otherMailbox # Start Modify CCopy
    ...
    $userObj | Add-Member -Type NoteProperty -Name $ccemailaddress -Value $ccemailaddress # Start Modify CCopy
...
$tenantId = "11111111-1111-1111-1111-000000008771"
   
#The Azure AD App id .
$appClientId="11111111-1111-1111-1111-000000008772"
...
function Send-mailOutha {
              ...
              $MessageParams = @{
                ...
                "Body" = (@{
                    ...
                    "ccRecipients" = @(
                        @{
                            "emailAddress" = @{"address" = $ccemailaddress }
                        } )
                    "attachments" = @(
                        @{
...
foreach ($user in $notifyUsers)
{
    ...
    $emailAddress = $user.EmailAddress
    # Start Modify CCopy
    $ccemailaddress = $user.ccemailaddress
    # End Modify CCopy
    ...
    <p> Dear user, <br><br>
    The automatic password expiration verification system has detected that your password in Telxiusapps domain expires in: <font color=#FF0000> $messageDays days </font> <br>
    ...
    <br>
    <p><b> We recommend you to change your password as soon as possible on the following link.</b></p>
    ...
    try{
        ...
        if($interval)
        {
            ...
            if(($interval) -Contains($daysToExpire))
            {
                ...
                # Start Modify CCopy
                if ([string]::IsNullOrEmpty($ccemailaddress)){Send-mailOutha -MsgFrom $mailFrom -EmailRecipient $emailaddress -MsgSubject $subject -HtmlMsg $body}
                else {Send-mailOutha -MsgFrom $mailFrom -EmailRecipient $emailaddress -MsgSubject $subject -HtmlMsg $body -ccemailaddress $ccemailaddress}
                # End Modify CCopy
