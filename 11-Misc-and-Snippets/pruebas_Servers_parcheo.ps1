...
$Output2 = Get-WMIObject  -Namespace root\sms\site_PE1 -class SMS_G_System_DCMDeploymentState -Filter "BaselineID = 'ScopeId_07A9F6D2-402D-4213-B63C-28D0A663EBC2/Baseline_a3fc6450-7488-4d34-a601-9471c53482a4'" | select ResourceName, ComplianceState, ResourceID
...
foreach($item in $DeviceLists)
    {
    $datosSCCM= Get-WmiObject -Namespace root\sms\site_PE1 -class SMS_R_System -Filter "ResourceId = $($item.ResourceId)" | select ResourceDomainORWorkgroup,OperatingSystemNameandVersion,Build
