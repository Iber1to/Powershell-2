$fileSalida="c:\temp\DO\APPsDO.csv"
$DC="ClientInsurance.com"
$DCINT="DCSRV01.int.ClientInsurance.net"
$baseDNs=@('OU=Service Accounts,DC=ClientInsurance,DC=com')
$baseDNsExcl=@()
...
$j=0
$i=0
"SamAccountName DO;SamAccountName INT;IGUALES;Nombre;Apellidos;Descripcion;Ultimo Logon;Habilitado;DN DO;DN INT" | Out-file $fileSalida
...
foreach ($ou in $OUs)
{
    ...
    if ($baseDNsExcl -notcontains $ou.DistinguishedName)
    {
        ...
        $datos=$null
        $datos= Get-ADUser -Filter * -SearchBase $ou.DistinguishedName -Properties samAccountName,givenname,sn,distinguishedname,Description,lastlogontimestamp,Enabled -server $DC `
        -SearchScope OneLevel | select samAccountName,givenname,sn,distinguishedname,Description,lastlogontimestamp,Enabled
        if ($datos -ne $null)
        {
            foreach ($item in $datos)
            {
                ...
                write-host ("$i"+" - "+$SAN+";"+$SANINT+";"+$IGUALES+";"+$givenName+";"+$sn+";"+$description+";"+$lastlogon+";"+$enabled+";"+$DN+";"+$DNINT) -f cyan
                write-output ($SAN+";"+$SANINT+";"+$IGUALES+";"+$givenName+";"+$sn+";"+$description+";"+$lastlogon+";"+$enabled+";"+$DN+";"+$DNINT)| out-file $fileSalida -Append
            }
        }
    }
    else
