...
# $usuarios = get-content C:\temp\irivalocaladmins.txt 
# Cuando hay pocos usuarios
 $usuarios = "user@contoso.com" #  si es un unico usuario.

#region Log
# Ruta del Archivo de log
$logFile = "C:\temp\Iriva.log"
...
$tenantId = "11111111-1111-1111-1111-000000008761"
$clientId = "11111111-1111-1111-1111-000000008763"
            ...
            @{
                ...
                settingInstance = @{
                    ...
                    settingInstanceTemplateReference = @{
                        settingInstanceTemplateId = "11111111-1111-1111-1111-000000008764"
                    }
                }
            }
        )
...
foreach ($usuario in $usuarios) {
    ...
    if ($null -ne $AzureObjectDevice){            
        ...
        if ($null -eq $UserGroupAd){
            ...
            } catch { Write-Log "Error al crear el grupo $groupName para el usuario $usuario"}
        } else { Write-Log "Existe el grupo $groupName para el usuario $usuario" }
        # Procesamos los dispositivos del usuario para agregarlos como miembros al grupo
        foreach ($item in $AzureObjectDevice){
            try {
                Add-DeviceToAzureGroup -GroupId $NewGroupUser.Id -DeviceId $item.Id -AccessToken $accessToken
                Write-Log "Dispositivo: $($item.DisplayName) agregado al grupo $groupName"
                } catch { Write-Log "Error al agregar el dispositivo $($item.DisplayName) al grupo $groupName"}
        }
        # Creamos la politica de protección de cuentas
        try {New-PolicyLocalAdmins -Usuario $usuario -AccessToken $accessToken -InternalGroupId $NewGroupUser.Id} catch { Write-Log "Error al crear la politica de protección de Cuentas para el usuario: $usuario" }
    } else {Write-Log "Error el usuario: $usuario no tiene dispositivo asociado"}    
}
