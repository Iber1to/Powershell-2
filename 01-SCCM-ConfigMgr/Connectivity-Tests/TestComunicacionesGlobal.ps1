...
if($testCCM){}else{mkdir C:\CCMScripts}
$StrCompliance= $null
...
Get-NetIPAddress -AddressFamily IPv4|Where-Object -Property InterfaceAlias -NotLike '*Pseudo-Interface*'| Sort-Object -Property InterfaceAlias  |ForEach-Object{Write-CMTracelog "$($_.InterfaceAlias): $($_.IPAddress)" } 
Write-Output  $(Get-NetIPAddress -AddressFamily IPv4|Where-Object -Property InterfaceAlias -NotLike '*Pseudo-Interface*'| Sort-Object -Property InterfaceAlias  |ForEach-Object{Write-Host "$($_.InterfaceAlias): $($_.IPAddress)" } )
...
If($MpServerName)
    {
        ...
        if($MpServerName -like "*CMG*"){$MpServerName = $MpServerName.Split('/')[0]}
        if($MpServerName -like "*CMG*")
            {
                $ManagementPorts = 443,10124,10140
            }
        else
            {
                $ManagementPorts = 80,135,445,10123
...
$DomainController= ([System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain()).Name
 If($DomainController)
    {
...
Write-Output "DC: $DomainController"    
Write-Output "Puertos DC error: $DomainControllerPortsError"


#Evaluamos si el equipo ha pasado el test
If(($null -eq $DomainControllerPortsError) -and ($null -eq $DistributionPortsError) -and ($null -eq $SUPPortsError) -and ($null -eq $ManagementPortsError) -and ($null -eq $LocalPortsError))
    {
...
$CheckPrimary= Test-Path '\\primary.contoso.com\Deploy_logs$'
If($CheckPrimary)
    {
        Copy-Item -Path $PathCMTracelog -Destination '\\primary.contoso.com\Deploy_logs$\TestPuertos'
    }
