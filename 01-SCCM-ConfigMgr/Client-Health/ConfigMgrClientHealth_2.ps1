...
Begin {
    ...
    Function Test-SCCMHardwareInventoryScan {
        ...
        if ($PowerShellVersion -ge 6) { $wmi = Get-CimInstance -Namespace root\ccm\invagt -Class InventoryActionStatus | Where-Object {$_.InventoryActionID -eq '{11111111-1111-1111-1111-000000000001}'} | Select-Object @{label='HWSCAN';expression={$_.ConvertToDateTime($_.LastCycleStartedDate)}} }
        else { $wmi = Get-WmiObject -Namespace root\ccm\invagt -Class InventoryActionStatus | Where-Object {$_.InventoryActionID -eq '{11111111-1111-1111-1111-000000000001}'} | Select-Object @{label='HWSCAN';expression={$_.ConvertToDateTime($_.LastCycleStartedDate)}} }
    ...
    # ref: https://social.technet.microsoft.com/Forums/de-DE/11111111-1111-1111-1111-000000000002/setup-was-unable-to-compile-the-file-discoverystatusmof-the-error-code-is-8004100e?forum=configmanagerdeployment
    ...
    Function Get-SCCMPolicySourceUpdateMessage {
        $trigger = "{11111111-1111-1111-1111-000000000003}"
    ...
    Function Get-SCCMPolicySendUnsentStateMessages {
        $trigger = "{11111111-1111-1111-1111-000000000004}"
        if ($PowerShellVersion -ge 6) { Invoke-CimMethod -Namespace 'root\ccm' -ClassName 'sms_client' -MethodName TriggerSchedule -Arguments @{sScheduleID=$trigger} -ErrorAction SilentlyContinue | Out-Null }
        else { Invoke-WmiMethod -Namespace 'root\ccm' -Class 'sms_client' -Name TriggerSchedule -ArgumentList @($trigger) -ErrorAction SilentlyContinue | Out-Null }
    }

    Function Get-SCCMPolicyScanUpdateSource {
        $trigger = "{11111111-1111-1111-1111-000000000005}"
        if ($PowerShellVersion -ge 6) { Invoke-CimMethod -Namespace 'root\ccm' -ClassName 'sms_client' -MethodName TriggerSchedule -Arguments @{sScheduleID=$trigger} -ErrorAction SilentlyContinue | Out-Null }
        else { Invoke-WmiMethod -Namespace 'root\ccm' -Class 'sms_client' -Name TriggerSchedule -ArgumentList @($trigger) -ErrorAction SilentlyContinue | Out-Null }
    }

    Function Get-SCCMPolicyHardwareInventory {
        $trigger = "{11111111-1111-1111-1111-000000000001}"
        if ($PowerShellVersion -ge 6) { Invoke-CimMethod -Namespace 'root\ccm' -ClassName 'sms_client' -MethodName TriggerSchedule -Arguments @{sScheduleID=$trigger} -ErrorAction SilentlyContinue | Out-Null }
        else { Invoke-WmiMethod -Namespace 'root\ccm' -Class 'sms_client' -Name TriggerSchedule -ArgumentList @($trigger) -ErrorAction SilentlyContinue | Out-Null }
    }

    Function Get-SCCMPolicyMachineEvaluation {
        $trigger = "{11111111-1111-1111-1111-000000000006}"
        if ($PowerShellVersion -ge 6) { Invoke-CimMethod -Namespace 'root\ccm' -ClassName 'sms_client' -MethodName TriggerSchedule -Arguments @{sScheduleID=$trigger} -ErrorAction SilentlyContinue | Out-Null }
        else { Invoke-WmiMethod -Namespace 'root\ccm' -Class 'sms_client' -Name TriggerSchedule -ArgumentList @($trigger) -ErrorAction SilentlyContinue | Out-Null }
    }
    ...
    <# Trigger codes
    {11111111-1111-1111-1111-000000000001} Hardware Inventory
    {11111111-1111-1111-1111-000000000007} Software Inventory
    {11111111-1111-1111-1111-000000000008} Discovery Inventory
    {11111111-1111-1111-1111-000000000009} File Collection
    {11111111-1111-1111-1111-000000000010} IDMIF Collection
    {11111111-1111-1111-1111-000000000011} Client Machine Authentication
    {11111111-1111-1111-1111-000000000012} Request Machine Assignments
    {11111111-1111-1111-1111-000000000006} Evaluate Machine Policies
    {11111111-1111-1111-1111-000000000013} Refresh Default MP Task
    {11111111-1111-1111-1111-000000000014} LS (Location Service) Refresh Locations Task
    {11111111-1111-1111-1111-000000000015} LS (Location Service) Timeout Refresh Task
    {11111111-1111-1111-1111-000000000016} Policy Agent Request Assignment (User)
    {11111111-1111-1111-1111-000000000017} Policy Agent Evaluate Assignment (User)
    {11111111-1111-1111-1111-000000000018} Software Metering Generating Usage Report
    {11111111-1111-1111-1111-000000000003} Source Update Message
    {11111111-1111-1111-1111-000000000019} Clearing proxy settings cache
    {11111111-1111-1111-1111-000000000020} Machine Policy Agent Cleanup
    {11111111-1111-1111-1111-000000000021} User Policy Agent Cleanup
    {11111111-1111-1111-1111-000000000022} Policy Agent Validate Machine Policy / Assignment
    {11111111-1111-1111-1111-000000000023} Policy Agent Validate User Policy / Assignment
    {11111111-1111-1111-1111-000000000024} Retrying/Refreshing certificates in AD on MP
    {11111111-1111-1111-1111-000000000025} Peer DP Status reporting
    {11111111-1111-1111-1111-000000000026} Peer DP Pending package check schedule
    {11111111-1111-1111-1111-000000000027} SUM Updates install schedule
    {11111111-1111-1111-1111-000000000028} NAP action
    {11111111-1111-1111-1111-000000000029} Hardware Inventory Collection Cycle
    {11111111-1111-1111-1111-000000000030} Software Inventory Collection Cycle
    {11111111-1111-1111-1111-000000000031} Discovery Data Collection Cycle
    {11111111-1111-1111-1111-000000000032} File Collection Cycle
    {11111111-1111-1111-1111-000000000033} IDMIF Collection Cycle
    {11111111-1111-1111-1111-000000000034} Software Metering Usage Report Cycle
    {11111111-1111-1111-1111-000000000035} Windows Installer Source List Update Cycle
    {11111111-1111-1111-1111-000000000036} Software Updates Assignments Evaluation Cycle
    {11111111-1111-1111-1111-000000000037} Branch Distribution Point Maintenance Task
    {11111111-1111-1111-1111-000000000038} DCM policy
    {11111111-1111-1111-1111-000000000004} Send Unsent State Message
    {11111111-1111-1111-1111-000000000039} State System policy cache cleanout
    {11111111-1111-1111-1111-000000000005} Scan by Update Source
    {11111111-1111-1111-1111-000000000040} Update Store Policy
    {11111111-1111-1111-1111-000000000041} State system policy bulk send high
    {11111111-1111-1111-1111-000000000042} State system policy bulk send low
    {11111111-1111-1111-1111-000000000043} AMT Status Check Policy
    {11111111-1111-1111-1111-000000000044} Application manager policy action
    {11111111-1111-1111-1111-000000000045} Application manager user policy action
    {11111111-1111-1111-1111-000000000046} Application manager global evaluation action
    {11111111-1111-1111-1111-000000000047} Power management start summarizer
    {11111111-1111-1111-1111-000000000048} Endpoint deployment reevaluate
    {11111111-1111-1111-1111-000000000049} Endpoint AM policy reevaluate
    {11111111-1111-1111-1111-000000000050} External event detection
    #>
