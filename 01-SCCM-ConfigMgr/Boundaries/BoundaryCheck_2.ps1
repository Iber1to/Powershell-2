...
$Listboundaries= Get-CMBoundary | where {$_.BoundaryType -eq 3}
...
foreach($ResourceID in $ListResourceID)
    {
     ...
     If($ResourceDevice.IPAddresses[0])
        {
        write-host Procesando $($ResourceDevice.name) con IP: $($ResourceDevice.IPAddresses[0])
         foreach($boundary in $Listboundaries)
            {
             ...
             if($BoundResult -eq $true)
                {
                 Write-Host "$($ResourceDevice.IPAddresses[0]) en $($boundary.DisplayName) es: $BoundResult" -ForegroundColor Green                 
                 Break    
                }
             else
                {
     ...
     #Los equipos que no tienen IP los reporto. Son buenos candidatos a reparar el cliente 
        ...
        {
         Write-Host " $($ResourceDevice.name) no tiene IP asociada" -ForegroundColor Red
         $MyUsuario = New-Object System.Object
         $MyUsuario | Add-Member -type NoteProperty -name 'Nombre maquina' -value $ResourceDevice.name
     ...
     If($FalseCount -eq $Listboundaries.Count)
        {
         Write-Host "$($ResourceDevice.IPAddresses[0]) no tiene boundary asociada" -ForegroundColor Red
         $MyUsuario = New-Object System.Object
         $MyUsuario | Add-Member -type NoteProperty -name 'Nombre maquina' -value $ResourceDevice.name
         $MyUsuario | Add-Member -type NoteProperty -name 'Ip maquina' -value $ResourceDevice.IPAddresses[0]      
...
$ListadoFinal | Export-Csv -Path C:\temp\equiposinboundary_$TargetCollection_$Date.csv -Append
$ListadoClientesRepairs | Export-Csv -Path C:\temp\clientesAreparar_$TargetCollection_$Date.csv -Append
