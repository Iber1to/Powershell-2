...
$IPAddress )
function Get-CMSiteCode { $CMSiteCode = Get-WmiObject -Namespace “root\SMS" -Class SMS_ProviderLocation -ComputerName $SiteServer | Select-Object -ExpandProperty SiteCode return $CMSiteCode } 
$Results = 0 
...
if ($BoundaryCount -ge 1) { 
                           $Boundary | ForEach-Object { $BoundaryName = $_.DisplayName 
                           ...
                           if (($ParseStartIP -le $ParseIP) -and ($ParseIP -le $ParseEndIP)) { 
                                                                                              if ($BoundaryName.Length -ge 1) {
                                                                                                                               $Results = 1 
                                                                                                                               Write-Output "`nIP address '$($IPAddress)' is within the following boundary:" Write-Output “Description: $($BoundaryName)`n"
                                                                                                                              } 
                                                                                              else {
                                                                                                    $Results = 1
...
if ($Results -eq 0) {
                    ...
                    else { Write-Output "`nNo IP range boundaries was found`n" }