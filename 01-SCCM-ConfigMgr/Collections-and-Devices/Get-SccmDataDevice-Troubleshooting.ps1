...
[CmdletBinding()] 
    param (
        ...
        [string]$SiteCode = "PE1",
        [string]$ProviderMachineName = "SRV002.contoso.local"               
          )
...
$TargetCollection= Get-Content 'C:\Temp\devicestroubleshooting.txt'

#Path para el export a CSV.
$PathCSV= 'C:\temp\sccmdataTroubles.csv'
<# Son equipos que no han parcheado no tiene sentido
...
foreach($item in $TargetCollection)
    {
        $GetCMdeviceData1= Get-CMDevice -name $item
        $GetCMdeviceData+= $GetCMdeviceData1
    }

$GetCMResourceData= @()
foreach($item in $GetCMdeviceData)
    {
        $GetCMResourceData1= Get-CMResource -ResourceId $item.ResourceID -Fast |Select-Object ResourceId,Build,BuildExt,Client,CPUType,CreationDate,FullDomainName,@{Name = 'IPAddresses';Expression = {$_.IPAddresses[0]}},@{Name = 'IPSubnets';Expression = {$_.IPSubnets[0]}},LastLogonTimestamp,LastLogonUserName 
        $GetCMResourceData+= $GetCMResourceData1
    }


#$GetSerialNumberData= Get-WmiObject -Namespace root\sms\site_CAS -Class SMS_G_System_PC_BIOS |Select-Object ResourceId, SerialNumber |Group-Object ResourceId -AsHashTable
$GetBatteryData= Get-WmiObject -Namespace root\sms\site_CAS -Class SMS_G_System_BATTERY |Select-Object ResourceId,BatteryStatus |Group-Object ResourceId -AsHashTable
$GetOperatinSytemData = Get-WmiObject -Namespace root\sms\site_CAS -Class SMS_G_System_OPERATING_SYSTEM | Select-Object ResourceId, caption, Version |Group-Object ResourceId -AsHashTable
...
$GetCMdeviceProperties= 'ADSiteName','ATPLastConnected','ClientCheckPass','ClientVersion','CoManaged','DeviceOS','DeviceOSBuild','EPAntivirusSignatureLastUpdateDateTime','EPAntivirusSignatureLastVersion','EPClientVersion','EPInfectionStatus','EPLastFullScanDateTimeEnd','EPLastInfectionTime','EPLastThreatName','EPPendingFullScan','LastActiveTime','LastHardwareScan','LastLogonUser','Name','ResourceID'
$GetCMResourceProperties= 'Build','BuildExt','Client','CPUType','CreationDate','FullDomainName','IPAddresses','IPSubnets','LastLogonTimestamp','LastLogonUserName'
...
Foreach($data in $GetCMdeviceData){
    ...
    foreach($prop in $GetCMResourceProperties){$DeviceMount | Add-Member -MemberType NoteProperty  -Name $prop -Value ($GetCMResourceData | Where-Object -Property ResourceId -eq $Data.ResourceId).$prop}
